...
function renderSeries(id){
 const s=catalog.find(x=>x.id===id); if(!s){renderCatalog();return}
 const eps=s.episodes||[];
 app.innerHTML=`<div class="topbar"><button class="back" id="back">← Series</button><div><div class="brand">${esc(s.title)}</div><div class="sub">Ficha de la serie</div></div></div>
 <img class="hero" src="${esc(abs(s.banner||s.poster))}" alt="Banner de ${esc(s.title)}">
 <h1 class="title">${esc(s.title)}</h1><div class="original">${esc(s.originaltitle||'')}</div>
 <div class="chips">${[s.country,s.year,s.rating?`⭐ ${s.rating}`:null,...(s.display_genres||s.genres||[])].filter(Boolean).map(x=>`<span class="chip">${esc(x)}</span>`).join('')}</div>
 <p class="plot">${esc(s.description||s.plot||'')}</p>
 <div class="info-grid">${info('Estado',s.status)}${info('Duración',s.runtime?`${s.runtime} min`:'')}${info('Estreno',s.premiered)}${info('Estudio',(s.studios||[]).join(', '))}</div>
 ${(s.cast||[]).length?`<h2 class="section-title">Reparto</h2><div class="cast">${s.cast.slice(0,12).map(a=>`<span>${esc(a.name)}${a.role?' · '+esc(a.role):''}</span>`).join('')}</div>`:''}
 <h2 class="section-title">Capítulos</h2><div id="episodes">${eps.length?eps.map((e,i)=>episodeHTML(e,i,s)).join(''):`<div class="empty">Todavía no hay capítulos registrados.</div>`}</div>`;
 document.getElementById('back').addEventListener('click',()=>setHash(''));
 app.querySelectorAll('[data-tg]').forEach(b=>b.addEventListener('click',()=>openTelegram(b.dataset.tg)));
}
